  ...
  <body>
    <main class="shell">
      <section class="hero">
        ...
        <h1>Claim a small amount of VELT for gas.</h1>
      ...
      <section class="panel panel-primary">
        <div class="panel-head">
          <div><div class="label">Claim</div><h2>Request testnet VELT</h2></div>
    ...
    <script>
      const state = { health: null, config: null, turnstileToken: "", widgetId: null, debug: { stage: 'boot', events: [] } };
      const els = {
      ...
      function pushDebug(label, extra = {}) {
        const entry = { at: new Date().toISOString(), label, ...extra };
        state.debug.events = [entry].concat(state.debug.events).slice(0, 8);
        const payload = {
          stage: state.debug.stage,
          hostname: window.location.hostname,
          origin: window.location.origin,
          userAgent: navigator.userAgent,
          siteKey: state.config?.turnstileSiteKey || '',
          widgetId: state.widgetId,
          tokenPresent: Boolean(state.turnstileToken),
          events: state.debug.events,
        };
        els.debugOutput.textContent = JSON.stringify(payload, null, 2);
        console.log('[Veltrix Faucet Debug]', entry);
      }
      ...
      async function renderTurnstile() {
        ...
        state.debug.stage = 'render-turnstile';
        pushDebug('render-start');
        if (!state.config.turnstileConfigured || !state.config.turnstileSiteKey) {
          ...
          pushDebug('missing-config', { configured: state.config.turnstileConfigured });
          return;
        }
        try {
          const turnstile = await ensureTurnstileReady();
          pushDebug('script-ready', { hasTurnstile: Boolean(turnstile) });
          state.widgetId = turnstile.render(els.turnstileWrap, {
            ...
            callback(token) {
              ...
              state.debug.stage = 'challenge-complete';
              pushDebug('challenge-success', { tokenLength: token?.length || 0 });
              setMessage('Challenge complete. You can claim now.', 'success');
            },
            'expired-callback'() {
              state.turnstileToken = '';
              state.debug.stage = 'challenge-expired';
              pushDebug('challenge-expired');
              setMessage('Challenge expired. Please retry it.', 'error');
            },
            'timeout-callback'() {
              state.turnstileToken = '';
              state.debug.stage = 'challenge-timeout';
              pushDebug('challenge-timeout');
              setMessage('Turnstile timed out. Retry the challenge.', 'error');
            },
            'error-callback'(errorCode) {
              state.turnstileToken = '';
              state.debug.stage = 'challenge-error';
              pushDebug('challenge-error', { errorCode, errorFamily: String(errorCode || '').slice(0, 3) });
              setMessage('Turnstile failed to load. Error code: ' + errorCode, 'error');
              return false;
            },
          });
          pushDebug('widget-rendered', { widgetId: state.widgetId });
          els.submit.disabled = false;
          setMessage('Complete the Turnstile challenge to enable claiming.');
        } catch (error) {
          state.debug.stage = 'render-error';
          pushDebug('render-exception', { message: error.message });
      ...
      async function refreshStatus() {
        ...
        try {
          ...
          state.debug.stage = 'status-loaded';
          updateCards();
          pushDebug('status-loaded', { turnstileConfigured: state.config.turnstileConfigured });
          await renderTurnstile();
        } catch (error) {
          state.debug.stage = 'status-error';
          pushDebug('status-error', { message: error.message });
      ...
      els.form.addEventListener('submit', async (event) => {
        ...
        try {
          ...
          pushDebug('submit-success', { txHash: payload.txHash });
          ...
          pushDebug('submit-error', { message: error.message });
          setMessage(error.message, 'error');
          els.submit.disabled = false;
        }
      });
      window.addEventListener('error', (event) => {
        pushDebug('window-error', { message: event.message, source: event.filename, line: event.lineno, column: event.colno });
      });
      window.addEventListener('unhandledrejection', (event) => {
        const reason = event.reason && event.reason.message ? event.reason.message : String(event.reason);
        pushDebug('unhandled-rejection', { reason });
      });
      refreshStatus();
    </script>
  </body>
