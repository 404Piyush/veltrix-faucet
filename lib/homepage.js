      ...
      async function renderTurnstile() {
        ...
        try {
          ...
          state.widgetId = turnstile.render(els.turnstileWrap, {
            ...
            theme: 'dark',
            callback(token) { state.turnstileToken = token; setMessage('Challenge complete. You can claim now.', 'success'); },
            'expired-callback'() { state.turnstileToken = ''; setMessage('Challenge expired. Please retry it.', 'error'); },
            'error-callback'() { state.turnstileToken = ''; setMessage('Turnstile failed to load. Refresh and try again.', 'error'); },
          });
